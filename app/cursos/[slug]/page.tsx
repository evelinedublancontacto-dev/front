    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/cursos" className="text-primary hover:underline font-body">Volver a cursos</Link>
        </div>
      </div>
    );
  }
                    ...
                    <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i} variants={fadeUp} className="bg-gradient-card rounded-xl border-glow overflow-hidden">
                      {video.url ? (
                        <div className="aspect-video">
                          <iframe src={video.url} title={video.title} className="w-full h-full" allowFullScreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" />
                        </div>
                      ) : (
                      ...
                      <div className="p-5 flex items-center justify-between">
                        ...
                        <span className="text-muted-foreground font-body text-sm flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> {video.duration}</span>
                      </div>
                    </motion.div>
                  ))}
                </div>
