...
export default function AdminCitasPage() {
  ...
  const fetchServicios = async () => {
    try {
      ...
      setServicios(records as unknown as Servicio[]);
    } catch (error: unknown) {
      const message =
        error instanceof Error ? error.message : "Error desconocido";
      console.error("Error al obtener servicios:", message);
    }
  };
  ...
  const fetchCitas = async () => {
    setLoading(true);
    try {
      const records = await pb.collection("citas").getList(1, 50, {
        sort: "-created",
        expand: "servicio",
      });
      ...
      const message =
        error instanceof Error ? error.message : "Error desconocido";
      console.error("Error al obtener citas:", message);
  ...
  const actualizarEstado = async (
    ...
    try {
      ...
      toast.success(`Cita ${ESTADO_LABELS[nuevoEstado].toLowerCase()}`);
      fetchCitas();
    } catch (error: unknown) {
      const message =
        error instanceof Error ? error.message : "Error desconocido";
      console.error("Error al actualizar cita:", message);
  ...
  const citasFiltradas = citas.filter((cita) => {
    ...
    const coincideBusqueda =
      ...
      cita.servicioNombre.toLowerCase().includes(busqueda.toLowerCase());
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500 mt-1">
                Administra las citas agendadas
              </p>
            </div>
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium border ${ESTADO_COLORS[cita.estado]}`}
                        >
